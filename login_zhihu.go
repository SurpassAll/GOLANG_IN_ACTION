...
const (
	...
	LOGIN   = "https://www.zhihu.com/login/phone_num"
	PROFILE = "https://www.zhihu.com/settings/profile"
)
...
func getLogin() {
	...
	if loginResp.StatusCode == 200 {
		...
		profileURL, _ := url.Parse(zhihuProfile)
		gCurCookies = loginResp.Cookies()

		gCurCookieJar.SetCookies(profileURL, gCurCookies)
		gCurCookieJar.Cookies(profileURL)
		profileClient := &http.Client{
			Jar: gCurCookieJar,
		}

		profileReq, _ := http.NewRequest("GET", zhihuProfile, nil)
		profileResp, err := profileClient.Do(profileReq)
		if err != nil {
			log.Fatal(err.Error())
		}
...
func main() {
	...
	// initAll()
	/**
	因为存在验证码 使用for死循环直到不需要验证码 再跳出循环进行下面的逻辑
	for {}
	*/
	// getLogin()
	...
	测试登陆页面
	*/
	resp, err := http.Get("https://www.zhihu.com/settings/profile")
	if err != nil {
		log.Fatal(err.Error())
	}
